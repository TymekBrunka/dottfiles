...
# alias askforperms='bash -c "$(declare -f askforperms); askforperms $1 $2 $3"'
function copylisted
{
	dirs=$(ls -d $2/* | grep ".$4") #every yello in .config
	cdirs=$(cat $3)
	inters=""
...
# copylisted=$(declare -f copylisted)
# alias copylisted='bash -c "$(declare -f copylisted); copylisted $1 $2 $3 $4"'
